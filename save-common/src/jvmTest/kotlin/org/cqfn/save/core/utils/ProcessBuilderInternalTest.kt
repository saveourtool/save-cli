...
class ProcessBuilderInternalTest {
    ...
    @Test
    fun `check stderr`() {
        val actualResult = processBuilder.exec("cd non_existent_dir", "", null, 10_000L, emptyList())
    ...
    fun `check stderr with additional warning`() {
        val actualResult = processBuilder.exec("cd non_existent_dir 2>/dev/null", "", null, 10_000L, emptyList())
        val expectedStdout: List<String> = emptyList()
        var expectedCode: Int
        lateinit var expectedStderr: List<String>
        when (getCurrentOs()) {
