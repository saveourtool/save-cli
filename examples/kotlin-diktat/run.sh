...
if ! [ -f ktlint ]; then
  ...
  curl -sSLO https://github.com/pinterest/ktlint/releases/download/$KTLINT_VERSION/ktlint >> temp.txt 2>&1
fi

if ! [ -f diktat-$DIKTAT_VERSION.jar ]; then
  echo "Downloading diktat-$DIKTAT_VERSION" >> temp.txt 2>&1
  curl -sSLO https://github.com/cqfn/diKTat/releases/download/v$DIKTAT_VERSION/diktat-$DIKTAT_VERSION.jar >> temp.txt 2>&1
fi

if ! [ -f save ]; then
  echo "Downloading SAVE" >> temp.txt 2>&1
  # Download latest release of SAVE CLI
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
      curl -sSL https://github.com/cqfn/save/releases/download/v$SAVE_VERSION/save-$SAVE_VERSION-linuxX64.kexe -o save >> temp.txt 2>&1
  elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
      curl -sSL https://github.com/cqfn/save/releases/download/v$SAVE_VERSION/save-$SAVE_VERSION-mingwX64.exe -o save >> temp.txt 2>&1
  fi
fi
